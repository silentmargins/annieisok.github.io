...
var Grid = (function() {
    ...
    Preview.prototype = {
        create : function() {
            ...
            this.$mediums = $('<p></p>');
            this.$dimensions = $('<p></p>');
            var possum = document.getElementsByTagName("a")[0].getAttribute("data-title");
            this.$uzsakymas = $('<p></p>').append('<div id="mygtukas"><button class="open-button" onclick="openForm()">Užsakyti šį spaudą</button><div class="form-popup" id="myForm"><form class="form-container" action="https://formspree.io/mayppbkw" method="POST"><label for="email"><b>Jūsų el. paštas</b></label>   <input type="text" placeholder="Įveskite paštą" name="email" required>    <label for="vardas"><b>Jūsų vardas</b></label>    <input type="text" placeholder="Įveskite vardą" name="vardas" required>		<label for="dydis"><b>Pasirinkite dydį</b></label>  <select id="dydis" placeholder="Variantai">    <option value="tapetas">A4 [21x30cm]</option><option value="plakatas">Plakatas [30x40cm]</option><option value="atvirukas">Atvirukas [10x15cm]</option>    <option value="lapas">Spec. [nurodykite žemiau]</option>  </select> <label for="kiekis"><b>Kiekis</b></label>    <input type="number" value="1" name="kiekis" min="1" max="5" required>		<label for="zinute">Nurodykite adresą ir/arba telefoną</label><textarea name="zinute"></textarea>   <button type="submit" class="btn">Užsakyti</button>    <button type="button" class="btn cancel" onclick="closeForm()">Uždaryti</button> </form></div>');
            this.$details = $('<div class="og-details"></div>').append(possum, this.$title, this.$description, this.$mediums, this.$dimensions, this.$uzsakymas);
